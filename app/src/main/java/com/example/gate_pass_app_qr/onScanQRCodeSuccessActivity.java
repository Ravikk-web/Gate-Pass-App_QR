...
//                                String passDetailsDocId = document.getString("passDetailsDocId");
    ...
    private void getTeacherName(String approvedBy) {
        ...
        refProfile.child(approvedBy).addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Log.w("Firebase Error", "Failed to read value:", error.toException());
                Toast.makeText(onScanQRCodeSuccessActivity.this, "Failed to read Value", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void SaveRecordDataToFireStore(String passRequestUserId, String passRequestUserDocId, String studentName, String studentId, String studentReason, Timestamp tokenGenerationTime, String PUid, String parentEmail, String parentMobile) {
        ...
        textView.setVisibility(View.VISIBLE);
        sendEmailToParent(parentEmail, studentName);
        textView.setText("An Email has been sent to the parent's Email.");
    ...
    private void sendEmailToParent(String parentEmail, String studentName) {
        Intent intent = new Intent(Intent.ACTION_SEND);
        intent.setType("text/plain"); // Plain text email
        intent.putExtra(Intent.EXTRA_EMAIL, new String[]{parentEmail});
        intent.putExtra(Intent.EXTRA_SUBJECT, "Important Information(Swift-Exit): Gate Pass Accepted");

        // Optional: Add email body content (if needed)
        intent.putExtra(Intent.EXTRA_TEXT, "This is a app generated email to inform the Parent of _"+studentName.toUpperCase()+"_ , that the 'gate pass' generated by the student has been successfully Redeemed.\n\nThe Student is now allowed to exit the College premise." +
                "\n(This email will be sent by the Gate-Keeper's registered email address, and can be modified)."+
                "\n\n*Thank You.* \n\nRegard: Don Bosco Institute of Technology, Bosco Technical Training Society.\n\ncopyright: @Swift-Exit-2024\nDeveloped by: Ravi Kant Kispotta ");

        startActivity(Intent.createChooser(intent, "Swift Exit: Gate pass 'Redeemed'"));
    }
